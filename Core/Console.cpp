...
void debug_outputbuffer(const uint16_t* buffer, int len)
{
	...
	for (int i = 0; i < len; i++)
	{
		...
		accumulator += num;
		// std::cout << std::setfill('0') << std::setw(2) << std::hex << int(ch) << " ";
	}
...
void Console::RunFrame()
{
	...
	while(_ppu->GetFrameCount() == frameCount) {
		...
		debug_outputbuffer(_ppu->GetCurrentOutputBuffer(), 256 * 240);
	}
}
